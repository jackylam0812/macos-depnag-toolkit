...
**登录后的注册提醒控制工具 · v2.1.0 · 中文网络恢复手册**

[源代码](https://github.com/jackylam0812/macos-depnag-toolkit) · [v2.1.0 下载](https://github.com/jackylam0812/macos-depnag-toolkit/releases/tag/v2.1.0) · [问题反馈](https://github.com/jackylam0812/macos-depnag-toolkit/issues)
...
| --- | --- | --- |
| 工具包 | `/Volumes/Data/Users/Shared/mdm-nag-recovery/toolkit-v2.1.0` | `/Users/Shared/mdm-nag-recovery/toolkit-v2.1.0` |
...
先可在浏览器查看[固定版本下载器源代码](https://github.com/jackylam0812/macos-depnag-toolkit/blob/v2.1.0/GET_TOOLKIT.sh)。以下命令只下载、改名并检查 shell 语法；**不会执行下载器**。失败的 `.part` 文件不会进入后面的执行步骤。
...
(
  ...
  /usr/bin/curl --fail --location \
    ...
    --output /tmp/GET_TOOLKIT-v2.1.0.sh.part \
    https://raw.githubusercontent.com/jackylam0812/macos-depnag-toolkit/v2.1.0/GET_TOOLKIT.sh
  /bin/mv /tmp/GET_TOOLKIT-v2.1.0.sh.part /tmp/GET_TOOLKIT-v2.1.0.sh
  /bin/sh -n /tmp/GET_TOOLKIT-v2.1.0.sh
...
(
  ...
  /bin/mkdir -p "$WORK"
  /bin/sh /tmp/GET_TOOLKIT-v2.1.0.sh "$WORK/toolkit-v2.1.0"
)
```

下载器从 [v2.1.0 Release](https://github.com/jackylam0812/macos-depnag-toolkit/releases/tag/v2.1.0) 获取 `macos-depnag-toolkit-v2.1.0.tar.gz` 及其 `.tar.gz.sha256`，解包后执行包自检，**不运行 `PREPARE.sh` 或 `APPLY_IN_RECOVERY.sh`**。
...
```sh
DATA="/Volumes/Data"
KIT="$DATA/Users/Shared/mdm-nag-recovery/toolkit-v2.1.0"
...
- 初始信任来自 **HTTPS、GitHub 账号 `jackylam0812` 和指定版本 tag**，不是来自独立签名。固定 `v2.1.0` 避免自动跟随 `main/latest`，但仓库管理者仍可能移动 tag 或替换 Release 资产。
...
```sh
KIT="/Users/Shared/mdm-nag-recovery/toolkit-v2.1.0"
...
sudo /bin/sh /tmp/GET_TOOLKIT-v2.1.0.sh \
  /Users/Shared/mdm-nag-recovery/toolkit-v2.1.0
```

成功后进入恢复模式，完整目录映射为 `/Volumes/Data/Users/Shared/mdm-nag-recovery/toolkit-v2.1.0`（按实际 Data 挂载点调整）。从第 5 节准备新会话继续。不要把正常系统的 `/Users/Shared/...` 在恢复模式里误认为目标安装的路径。
...
- **v2.1.0 新增固定版本网络获取；通用动态会话、下载流程的实际恢复环境与重启流程仍需按本手册逐项验收。** 仓库的隔离测试使用合成数据，不会修改本机原生文件，不是全面抹盘的端到端测试。
- `sha256-file` 为 arm64 / x86_64 universal Mach-O，仅依赖系统 `libSystem`。架构分片存在不等于两个架构都已完成真实恢复模式实测。
- 具体已执行的测试和结果见随包 `VERIFICATION.txt` 及 `tests/`；示例数据不代表任何用户真实配置。
